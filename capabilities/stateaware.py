...
class StateAware(AgentCapability):
    """
    StateAware is an agent capability that allows an agent to be aware of the state of the conversation.
    To make any conversable agent state-aware, instantiate both the agent and the StateAware class,
    then pass the agent to state_aware.add_to_agent(agent).
    Note that state-aware agents in a group chat must be given unique path_to_db_dir values.
    
    When adding StateAware to an agent, the following are modified:    
    ...
    and the hook potentially modifies the last of the received messages to determine the completed tasks to the message.
    """
    ...
    def add_to_agent(self, agent: ConversableAgent):
        """Adds state aware ability to the given agent."""
        ...
        assert self.llm_config, "StateAware ability requires a valid llm_config."
        ...
        agent.update_system_message(
            ...
            + "\nYou've been given the special ability to record each task/step you have completed from the provided plan. You can retrieve this list prior to a conversation to understand which tasks you need to do and which tasks you can skip based on memory."
        )

    def process_last_received_message(self, text: Union[Dict, str]):
        """
        Appends any relevant completed tasks to the message text, and stores it in the memo db.
        Uses TextAnalyzerAgent to make decisions about task memo storage and retrieval.
        """

        # Try to retrieve relevant tasks from the DB.
        expanded_text = text
    ...
    def _consider_memo_storage(self, comment: Union[Dict, str]):
        ...
        if "yes" in response.lower():
            # Can we extract the task?
            advice = self._analyze(
                comment,
                "Briefly determine if the task was completed and if so then copy just the task from the TEXT. But if no task is completed, just respond with 'none'.",
            ...
            if "none" not in advice.lower():
                ...
                if self.verbosity >= 1:
                    ...
                    print(colored(f"\nTASK: {general_task} ||| Advice: {advice}", "light_yellow"))
    ...
    def _consider_memo_retrieval(self, comment: Union[Dict, str]):
        """Decides whether to retrieve completed tasks from the DB, and add them to the chat context."""

        # First, use the comment directly as the lookup key.
        if self.verbosity >= 1:
            print(colored("\nLOOK FOR COMPLETED TASKS", "light_yellow"))
    ...
    def _retrieve_relevant_memos(self, input_text: str) -> list:
        """Returns semantically related Tasks from the DB."""
...
class MemoStore:
    ...
    def get_related_memos(self, query_text: str, n_results: int, threshold: Union[int, float]):
        """Retrieves tasks that are related to the given query text within the specified distance threshold."""
